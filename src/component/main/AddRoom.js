...
export default class AddRoom extends React.Component {
    ...
    componentWillMount() {
        AsyncStorage.getItem('id').then((id) => {
            this.setState({ID: id},()=>        this._GetFiles(id)
        )
        })
    }
        ...
        return (
            <View style={styles.MainView}>
                <ScrollView style={{flex: 1}} showsVerticalScrollIndicator={false}>
                    <Header navigation={this.props.navigation} PageTitle={'افزودن اتاق'}/>
                    ...
                    <TouchableOpacity style={[styles.AddBtn, {}]} onPress={() => this._AddRoom()}>
                        <Text
...
//---------------------Get AmlakLIst --------------------------------------------------------------------------------------
    ...
    _AddRoom() {
        ...
        if (1) {
            ...
            Connect.SendPRequest(URLS.Link() + "addroom", {
                userId: parseInt(ID),
                file:FileId,
                name: name,
                zarfiat:capacity,
